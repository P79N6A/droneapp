...
public final class c {
    ...
    private static boolean a(StringBuffer stringBuffer) {
        try {
            return new JSONObject(stringBuffer.toString()).getJSONObject(com.google.firebase.a.a.b.p).getDouble("accuracy") < 5000.0d;
        } catch (Exception e) {
            return false;
        }
    }
    ...
    private boolean a(List<ScanResult> list) {
        ...
        int size = list.size();
        return (size < 6 || i < (size / 2) + 1) ? (size >= 6 || i < 2) ? this.d.size() <= 2 && list.size() <= 2 && Math.abs(System.currentTimeMillis() - this.b) <= NotificationOptions.b : true : true;
    }
    ...
    public final String b(int i, int i2, int i3, int i4, List<ScanResult> list) {
        ...
        if (str == null || list == null) {
            ...
            if ((abs > NotificationOptions.b && list.size() > 2) || ((abs > 45000 && list.size() <= 2) || !a(new StringBuffer(str)))) {
                str = null;
            }
        }
        this.e = str;
