...
public class SmileFactory extends JsonFactory {
    static final int DEFAULT_SMILE_GENERATOR_FEATURE_FLAGS = Feature.collectDefaults();
    ...
    protected SmileGenerator _createJsonGenerator(OutputStream outputStream, IOContext iOContext) {
        ...
        if ((Feature.WRITE_HEADER.getMask() & i) != 0) {
            smileGenerator.writeHeader();
        } else if ((Feature.CHECK_SHARED_STRING_VALUES.getMask() & i) != 0) {
        ...
        } else if ((Feature.ENCODE_BINARY_AS_7BIT.getMask() & i) == 0) {
    ...
    public SmileParser createJsonParser(byte[] bArr) {
        return _createJsonParser(bArr, 0, bArr.length, _createContext(bArr, true));
    }
