package com.google.android.gms.wearable.internal;

import android.os.Parcel;
import android.os.Parcelable.Creator;
import com.google.android.gms.common.internal.safeparcel.a;
import com.google.android.gms.common.internal.safeparcel.b;

public class n implements Creator<GetCloudSyncOptInStatusResponse> {
    static void a(GetCloudSyncOptInStatusResponse getCloudSyncOptInStatusResponse, Parcel parcel, int i) {
        int a = b.a(parcel);
        b.a(parcel, 1, getCloudSyncOptInStatusResponse.a);
        b.a(parcel, 2, getCloudSyncOptInStatusResponse.b);
        b.a(parcel, 3, getCloudSyncOptInStatusResponse.c);
        b.a(parcel, 4, getCloudSyncOptInStatusResponse.d);
        b.a(parcel, a);
    }

    public GetCloudSyncOptInStatusResponse a(Parcel parcel) {
        boolean z = false;
        int b = a.b(parcel);
        boolean z2 = false;
        int i = 0;
        int i2 = 0;
        while (parcel.dataPosition() < b) {
            int a = a.a(parcel);
            switch (a.a(a)) {
                case 1:
                    i2 = a.g(parcel, a);
                    break;
                case 2:
                    i = a.g(parcel, a);
                    break;
                case 3:
                    z2 = a.c(parcel, a);
                    break;
                case 4:
                    z = a.c(parcel, a);
                    break;
                default:
                    a.b(parcel, a);
                    break;
            }
        }
        if (parcel.dataPosition() == b) {
            return new GetCloudSyncOptInStatusResponse(i2, i, z2, z);
        }
        throw new a.a("Overread allowed size end=" + b, parcel);
    }

    public GetCloudSyncOptInStatusResponse[] a(int i) {
        return new GetCloudSyncOptInStatusResponse[i];
    }

    public /* synthetic */ Object createFromParcel(Parcel parcel) {
        return a(parcel);
    }

    public /* synthetic */ Object[] newArray(int i) {
        return a(i);
    }
}
